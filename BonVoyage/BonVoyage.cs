...
namespace BonVoyage
{
	...
	*/
	...
	public class BonVoyage : MonoBehaviour
	{
		...
		public void UpdateRoverState(Vessel vessel, bool stateActive)
		{
			for (int i = 0; i < activeRovers.Count; i++)
			{
				...
				if (rover.vessel == vessel)
				{
					...
					{
						if(!(rover.kerbalismFuelCell))
							LqdFuelOxiDrain(vessel, rover.stockFuelCellECMax - rover.stockFuelCellECRemain);
						InputLockManager.RemoveControlLock("BonVoyageInputLock");
					}
					return;
				}
		...
		private void LqdFuelOxiDrain(Vessel vessel, double ecToDrain)
		{
			double lqdFuelRemainder = ecToDrain * 0.001125;
			double oxiRemainder = ecToDrain * 0.001375;
			var partList = vessel.parts;

			for (int i = 0; i < partList.Count; i++)
			{
				var resourceList = partList[i].Resources;
				for (int j = 0; j < resourceList.Count; j++)
				{
					if (resourceList[j].resourceName == "LiquidFuel")
					{
						if (resourceList[j].amount >= lqdFuelRemainder)
						{
							resourceList[j].amount -= lqdFuelRemainder;
						...
						{
							lqdFuelRemainder -= resourceList[j].amount;
							resourceList[j].amount = 0.0;
						}
					}

					if (resourceList[j].resourceName == "Oxidizer")
					{
						if (resourceList[j].amount >= oxiRemainder)
						{
							resourceList[j].amount -= oxiRemainder;
							oxiRemainder = 0.0;
						...
						{
							oxiRemainder -= resourceList[j].amount;
							resourceList[j].amount = 0.0;
						}
					}
				}
				if (oxiRemainder == 0.0 && lqdFuelRemainder == 0.0)
					break;
			}
		}
	}
